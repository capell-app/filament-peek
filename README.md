...
Individual package prerequisites are noted in each package README.

---

## Mosaic (Layout)

Visual layout builder with widget management. Provides the drag-and-drop page builder, Filament resources for Contents and Widgets, and the frontend rendering pipeline. Every other add-on builds on Mosaic — install it first.
...
Country and address models with Filament resources. Attaches address and country relationships to `Site` at runtime — no schema changes to the core `sites` table.
