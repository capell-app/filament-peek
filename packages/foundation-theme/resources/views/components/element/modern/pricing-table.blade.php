@props([
    'currency' => $element->getMeta('currency', '$'),
    'billingOptions' => $element->getMeta('billing_options', 'monthly'),
    'container',
    'containerKey',
    'containerWidth' => null,
    'loop',
    ...
    <section class="px-6 py-12 md:px-12 md:py-16">
        ...
        >
            @forelse ($element->assets as $elementAsset)
                @php
                    $price = $elementAsset->asset->getMeta('price', '0');
                    $priceAnnual = $elementAsset->asset->getMeta('price_annual', $price);
                    $featured = (bool) $elementAsset->asset->getMeta('featured', false);
                    $ctaLabel = $elementAsset->asset->getMeta('cta_label', 'Get Started');
                    $ctaUrl = $elementAsset->asset->getMeta('cta_url', '#');
                    $features = $elementAsset->asset->getMeta('features', []);
                @endphp
