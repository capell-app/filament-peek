...
@if ($block->assets->isNotEmpty() || ! config('capell-layout-builder.block.skip_render_empty', true))
    <x-capell-foundation-theme::block.wrapper
        class="capell-asset-accordion block-section-grid block-accordion space-y-6"
        :$container
        :$containerKey
        :$containerWidth
        :index="$loop->index"
    ...
    >
        ...
        @if ($block->assets->isNotEmpty())
            ...
            >
                ...
                {{-- format-ignore-end --}}
                    <section
                        class="block-accordion-item flex flex-col gap-1 bg-gray-50 py-3 first:rounded-t-lg last:rounded-b-lg dark:bg-white/5"
                    >
                        <button
                            type="button"
                            x-on:click="
