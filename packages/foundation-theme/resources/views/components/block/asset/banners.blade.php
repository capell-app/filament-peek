...
@php
    $carouselId = sprintf('banner-carousel-%s-%s', $block->id ?? $block->key, $loop->index);
    $bannerItems = BuildAssetBannerItemsAction::run($block);
    $total ??= $bannerItems->count();
@endphp

@if ($bannerItems->isNotEmpty() || ! config('capell-layout-builder.block.skip_render_empty', true))
    <section
        class="capell-asset-banners block-assets-banner relative flex w-full items-center justify-center overflow-hidden"
        style="
        ...
        >
            <div class="swiper-wrapper h-full w-full">
                @foreach ($bannerItems as $bannerItem)
                    ...
                    <div
                        @class([
                            'swiper-slide block-banner-item relative flex min-h-[20rem] w-full shrink-0 basis-full items-center justify-center',
