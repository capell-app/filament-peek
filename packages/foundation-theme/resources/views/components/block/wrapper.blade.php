...
@props([
    'backgroundAttachment' => $block->getMeta('background_attachment'),
    'backgroundColor' => $block->getMeta('background_color'),
    'backgroundImage' => ResolveLoadedBlockBackgroundImageAction::run($block),
    'backgroundRepeat' => $block->getMeta('background_repeat', 'no-repeat'),
    'backgroundOverlay' => (bool) $block->getMeta('background_overlay'),
    'backgroundPosition' => $block->getMeta('background_position', 'center'),
    'backgroundSize' => $block->getMeta('background_size', 'cover'),
    'class' => '',
    'container',
    'containerKey',
    'containerClass' => '',
    'containerWidth' => GetBlockContainerWidthAction::run($block),
    'index',
    'margin' => Arr::wrap($block->getMeta('margin')),
...
@php
    ...
    if ($block->getMeta('container') !== null || ! $containerWidth instanceof ContainerWidthEnum) {
        $containerWidth = GetBlockContainerWidthAction::run($block);
    }
@endphp
...
<{{ $tag }}
    id="{{ $containerKey . '-' . $block->key . "-{$index}" }}"
    {{
        $attributes->class([
            '@container block block-' . $block->key,
            $class => $class !== 'block-' . $block->key,
...
>
    @if ($backgroundOverlay)
        <div
            class="capell-block-wrapper absolute inset-0 z-0 bg-black/40 shadow-[inset_0_0_8rem_4rem_rgba(0,0,0,0.7)]"
        ></div>
    @endif
