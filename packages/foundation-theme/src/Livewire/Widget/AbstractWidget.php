...
abstract class AbstractWidget extends Component
{
    ...
    {
        try {
            $reference = json_decode(Crypt::decryptString($this->widgetReference), true, flags: JSON_THROW_ON_ERROR);
        } catch (Throwable) {
            $reference = [];
        }
    ...
    private function resolveScopedWidget(): ?Element
    {
        $layout = $this->currentLayout();
        $language = $this->currentLanguage();
    ...
    private function widgetData(): array
    {
        ...
        foreach (LayoutElementData::normalizeMany($this->containerData()['elements'] ?? []) as $layoutElementData) {
            if (
                ...
                return array_merge($layoutElementData, $this->referenceWidgetData, $widgetData);
            }
        }
