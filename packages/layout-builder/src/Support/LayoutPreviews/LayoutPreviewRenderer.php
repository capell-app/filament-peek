...
class LayoutPreviewRenderer
{
    ...
    public function render(Layout $layout): string
    {
        if (! function_exists('imagecreatetruecolor')) {
            throw new RuntimeException('The GD extension is required to generate layout previews.');
        }
        ...
        if (! is_string($png) || $png === '') {
            throw new RuntimeException('Unable to encode layout preview PNG.');
        }
    ...
    {
        $containerColor = $this->color($layout, 'container:' . (string) ($container['key'] ?? 'container'), 0.22, $usedHues);
    ...
    {
        $hash = crc32((string) $layout->getKey() . ':' . $layout->key . ':' . $key);
