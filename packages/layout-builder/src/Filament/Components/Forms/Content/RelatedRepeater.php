...
namespace Capell\LayoutBuilder\Filament\Components\Forms\Content;

use Capell\LayoutBuilder\Actions\ReplicateContentAction;
use Capell\LayoutBuilder\Filament\Components\Forms\ContentSelect;
...
class RelatedRepeater
{
    public static function make(Schema $configurator): Repeater
    {
        return Repeater::make('related')
            ->label(__('capell-layout-builder::form.related_contents'))
            ->statePath('related')
            ->hiddenLabel()
            ->cloneable()
            ->cloneAction(
