...
use Capell\BlockLibrary\Filament\Components\Tables\Columns\Content\ContentNameColumn;
...
class BlockLibraryTable implements TableConfigurator
{
    public static function configure(Table $table): Table
    {
        return $table
            ...
            ->recordClasses(fn (ContentBlock $record): ?string => match (true) {
    ...
    protected static function getTableColumns(): array
    {
        return [
            ...
            TextColumn::make('children_count')
                ->label(__('capell-block-library::table.children'))
                ->alignCenter()
                ->numeric()
                ->sortable()
                ->toggleable()
                ->color('primary')
                ->url(function (ContentBlock $record, int $state): ?string {
                    if ($state === 0) {
                        return null;
                    }
            ...
            BadgeableColumn::make('assets_count')
                ->label(__('capell-block-library::table.assets'))
                ->alignCenter()
                ->numeric()
                ->sortable()
                ->toggleable()
            ...
            DateColumn::make('visible_from')
                ->label(__('capell-block-library::table.visible_from'))
                ->toggleable(isToggledHiddenByDefault: true),
            DateColumn::make('visible_until')
                ->label(__('capell-block-library::table.visible_until'))
                ->toggleable(isToggledHiddenByDefault: true),
    ...
    protected static function getTableFilters(): array
    {
        return [
            ...
            SelectFilter::make('type_id')
                ...
                ->relationship(
                    ...
                    modifyQueryUsing: fn (Builder $query): Builder => $query->where(
                        'type',
                        LayoutTypeEnum::ContentBlock->value,
                    )
                        ->enabled(),
                ),
                ...
                ->schema([
                    ...
                    Select::make('parent_id')
                        ...
                        ->options(function (HasTable $livewire, Get $get): array {
                            ...
                            /** @var class-string<ContentBlock> $model */
                            $model = ContentBlock::class;

                            $block_library = $model::with([
                                'site',
                                'ancestors',
                            ])
                                ->whereHas('children')
                            ...
                            return $block_library->mapWithKeys(function (ContentBlock $content_block) use ($siteId): array {
                                ...
                                $ancestors = $content_block->ancestors()->get();
                                ...
                                $label .= Str::limit($content_block->name, 40);

                                return [$content_block->id => $label];
                            })
                                ->all();
                        }),
                ])
            ...
            SelectFilter::make('publish_status')
                ->label(__('capell-block-library::table.publish_status'))
