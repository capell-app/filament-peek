...
namespace Capell\BlockLibrary\Filament\Concerns;
...
trait HasAssetsRelationManager
{
    protected static function createResourcesAction(): Action
    {
        return CreateAction::make()
            ->label(__('capell-block-library::button.add_asset'))
            ->color('primary')
            ->successNotificationTitle(__('capell-block-library::message.asset_added'))
            ->using(function (array $data, self $livewire): Model {
                throw_if(! isset($data['asset_id']), RuntimeException::class, 'No asset selected');
                throw_unless($livewire->ownerRecord instanceof ContentBlock, RuntimeException::class, 'Invalid asset owner.');
                ...
                function (Select $select) use ($asset): Select {
                    ...
                    return $select->createOptionForm(
                        ...
                        ->createOptionUsing(function (Select $component, array $data) use ($asset, $adminAsset, $createOptionUsing): int|string {
                            ...
                            Notification::make()
                                ->title(__('capell-block-library::message.asset_created_successfully', ['name' => $asset->name]))
                                ->body($page->name)
                                ->send();
