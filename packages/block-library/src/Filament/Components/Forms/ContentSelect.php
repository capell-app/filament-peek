...
class ContentSelect extends Select
{
    ...
    private function getContentOptions(?int $siteId = null, ?string $search = null): array
    {
        ...
        $contents = ContentBlock::query()
            ->select('block_library.*')
            ->with($relations)
            ->join('types', 'block_library.type_id', '=', 'types.id')
            ->when(
            ...
            ->when(
                $search,
                fn (Builder $query, string $search): Builder => $query->where('block_library.name', 'like', sprintf('%%%s%%', $search))
                    ->orderByRaw('CASE WHEN block_library.name = ? THEN 1 ELSE 0 END DESC, INSTR(block_library.name, ?), block_library.name', [$search, $search]),
