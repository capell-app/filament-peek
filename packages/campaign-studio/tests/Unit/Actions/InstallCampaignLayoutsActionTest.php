...
it('installs campaign layouts with layout-builder compatible block references', function (): void {
    ...
    $blocks = $layout->getAttribute('blocks');

    expect($result)->toBe(['created' => 3, 'updated' => 0, 'skipped' => 0])
        ->and($containers)->toHaveKeys(['hero', 'proof', 'form'])
        ->and($containers['hero']['blocks'][0])->toMatchArray([
            'block_key' => 'campaign-lead-generation-campaign-hero',
            'occurrence' => 1,
        ])
        ->and($blocks)->toContain('campaign-lead-generation-campaign-hero')
        ->and(Block::query()->where('key', 'campaign-lead-generation-campaign-hero')->exists())->toBeTrue()
        ->and(Block::query()->where('key', 'campaign-lead-generation-campaign-cta-block')->exists())->toBeTrue()
        ->and(Block::query()->where('key', 'campaign-lead-generation-campaign-lead-form')->exists())->toBeTrue();
});
