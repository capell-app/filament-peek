...
final class PhoneHomeAction
{
    ...
    public function handle(): bool
    {
        ...
        if ($baseUrl === null || $baseUrl === '') {
            Log::warning('capell-extension-marketplace: marketplace.base_url not configured, skipping phone home.');
        ...
        if ($webhookUrl === null) {
            Log::warning('capell-extension-marketplace: marketplace webhook URL is not configured, skipping phone home.');
        ...
        $instanceId = $marketplaceInstance?->instance_id ?? config('capell-extension-marketplace.instance.id');

        if (! is_string($instanceId) || $instanceId === '') {
            $this->failureMessage = 'This installation is not connected to Capell Marketplace. Connect and verify the domain before running heartbeat.';
            self::$lastFailureMessage = $this->failureMessage;
        ...
        try {
            $heartbeatResult = $this->marketplace->heartbeat([
            ...
            Log::warning('capell-extension-marketplace: phone home connection failed', ['error' => $connectionException->getMessage()]);
            $this->failureMessage = 'Capell could not connect to the marketplace API. Heartbeat URL: '
                . $heartbeatUrl
                . '. Error: '
                . $connectionException->getMessage();
        } catch (RuntimeException $runtimeException) {
            Log::warning('capell-extension-marketplace: phone home failed', ['error' => $runtimeException->getMessage()]);
